...
describe('search-router endpoints', () => {
    ...
    context('Given there are reviews in the database', () => {
        ...
        it(`GET /api/search responds with 200 and all of the reviews`, () => {
            return supertest(app).get('/api/search').expect(200, testReviews);
        });
        ...
        it(`GET /api/search/term responds with 200 and the matched cities`, () => {
            ...
            const expectedReview = testReviews.filter(
                review => review.city === newSearch.city
            );
            return supertest(app)
                .get('/api/search/term')
                .query(newSearch)
    ...
    describe('Given no reviews in the database ', () => {
        it(`GET /api/search responds with 200 and an empty list`, () => {
            return supertest(app).get('/api/search').expect(200, []);
        });
        ...
        it(`GET /api/search/term responds with 404 and an error`, () => {
            ...
            return supertest(app)
                ...
                .expect(404, {
                    error: {
                        message:
                            'City with that category has not been reviewed yet',
                    },
                });
        });
    });
    ...
    context('Given an xss attack', () => {
        ...
        const { maliciousSearch, expectedSearch } = helpers.makeMaliciousReview(
            testUser
        );
