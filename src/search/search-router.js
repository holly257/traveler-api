...
searchRouter.route('/').get((req, res, next) => {
...
searchRouter.route('/term').get((req, res, next) => {
    const db = req.app.get('db');
    ...
    if (!city) {
        return res
            .status(400)
            .json({ error: { message: 'City name must be included' } });
    }
    ...
    if (!category) {
        ...
        SearchReviewsService.getBySearchTermAndCategory(
            db,
            search_city,
            search_cat
        )
